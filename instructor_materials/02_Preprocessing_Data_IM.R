...
# VERY IMPORTANT: order of samples must be identical in count and sample matrices
...
# for each gene,
# 1) compute geometric mean across samples (not as sensitive to outlier as arithmetic mean) 
# 2) divide count in each sample by that mean
# for each sample,
# compute median of resulting counts across all genes: the scaling factor

ddsObj <- estimateSizeFactors(ddsObj)
sizeFactors(ddsObj)

# less than 1: lib size will be scaled down, ie ~ read counts increased
# greater than 1: lib size wil be scaled up, ie ~ read counts decreased

# MAplot to compare one sample against all others
# diff vs expression level
# limma
