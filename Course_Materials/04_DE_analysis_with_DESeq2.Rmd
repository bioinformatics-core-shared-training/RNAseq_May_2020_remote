...
model contrasting the differential expression between cell types.
We can change the model later if we want to.
...
> ### Exercise 1
>
> Plot the biased and unbiased MA plots for both samples side by side to see the 
> effect of normalisation.
>

```{r solutionExercise1, echo=FALSE, fig.height=10, fig.width=10}
...
>    `~CellType + Status + CellType:Status`, where `CellType:Status` is the 
>     parameter for the interaction beteween CellType and> Status.
>
