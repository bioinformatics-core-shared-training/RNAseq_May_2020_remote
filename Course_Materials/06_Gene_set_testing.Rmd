...
difference of interest between samples [@Subramanian15545]. A gene set comprises
genes that share a biological function, chromosomal location, or any other
relevant criterion.
...
The Broad also provide a number of very well curated `gene set`s
...
The function `plotGseaTable` allows us to plot a summary figure showing the 
...
```{r goSeqPackage, echo=FALSE, message=FALSE, warning=FALSE}
library(goseq)
...
![mmu03320 - PPAR signaling pathway](/home/ubuntu/Course_Materials/results/mmu03320.pathview.png)
