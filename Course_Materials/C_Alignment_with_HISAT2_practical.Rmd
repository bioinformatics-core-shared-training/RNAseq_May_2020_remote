...
>    later.
> 4. Make a directory (`mkdir`) called `hisat2_index_chr1` inside the 
>    `references` directory.  This is where we will create our chr1 index.
...
The curly brackets (`{...|...}`) indicate that we can either use two arguments
`-1` and `-2` to give two fastq files when we have paired end reads, or we can
use `-U` to just give one fastq if our reads are single end, as is the case 
today.

The square brackets (`[...]`) indicate that argument `-S` is entirely optional.
This argument is used to provide a file to write the results to. If we do not
provide this the default is to write to `stdout`, which refers to the "standard
output" and means the results would just be printed on the screen.
...
> of many unix/linux tools). The alignment take 10-15 minutes - go get a nice 
> cup of tea.
